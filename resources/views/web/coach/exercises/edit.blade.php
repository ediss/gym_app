        ...
        <div class="col-lg-12">
            <div class="bg-transparent">
                <div class="card-body p-4">
                    <h5 class="mb-4 text-warning text-center">Edit Exercise</h5>
                    <form class="row g-3" method="POST" action="{{ route('exercise.update', ['exercise' => $exercise->id]) }}">
                        @csrf
                            ...
                            <div class="input-group">
                                <label class="form-label w-100">
                                    <input name="name" type="text" placeholder="Name"
                                        class="form-control bg-transparent {{ $errors->has('name') ? 'border-danger' : '' }}"
                                        value="{{ $exercise->name }}">
                                </label>
                            </div>
                                    ...
                                    <select class="form-select" name="exercise_category_id">
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                {{  $exercise->exercise_category_id === $category->id ? 'selected' : '' }}>
                                    ...
                                    <select class="form-select" name="exercise_type_id">
                                        @foreach ($types as $type)
                                            ...
                                            <option value="{{ $type->id }}"
                                                {{  $exercise->exercise_type_id === $type->id ? 'selected' : '' }}>
                                                {{ $type->name }}</option>
                                        @endforeach
                                    </select>
                                </label>
